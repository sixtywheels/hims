    ...
    methods: {
        ...
        login: async function() {
            if (this.email == null|| this.password==null  ){
                ...
                var data = await getDocs(query(collection(db, "users"), where("email", "==", this.email)));
                if(data.empty) {
                    alert("No such company email exist in the database. If you are a L&P staff, please proceed to the L&P login page!")
                } else {
                    await firebase
                        .auth()
